...
import { parseToPlanDraft, summarizeDraft } from '@/services/planParser'
...
async function handleFileUpload(event: Event) {
  ...
  for (const file of Array.from(files)) {
    ...
    if (result.content && !result.content.startsWith('data:')) {
      smartInput.value = result.content
    }
    if (result.warning) {
  ...
  <div class="space-y-6 pb-20 md:pb-0 max-w-2xl">
    ...
    <div v-if="planDraft && draftSummary" class="bg-gradient-to-r from-accent/5 to-blue-50/30 border border-accent/15 rounded-2xl p-5">
      ...
      <div class="space-y-3 max-h-60 overflow-y-auto">
        <div v-for="(tasks, day) in Object.entries(draftSummary.byDay).sort((a,b) => +a[0] - +b[0])" :key="day">
          <p class="text-xs text-text-muted mb-1">第 {{ tasks[0] }} 天 · {{ planDraft.tasks.find(t => t.day === +tasks[0])?.date || '' }}</p>
          <div v-for="t in planDraft.tasks.filter(t => t.day === +tasks[0])" :key="t.title" class="flex items-center gap-2 text-xs py-1 px-2 bg-white rounded">
            ...
            <span class="flex-1 text-text-primary truncate">{{ t.title }}</span>

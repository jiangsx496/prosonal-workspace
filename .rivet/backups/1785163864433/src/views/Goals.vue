...
const deleteWithTasks = ref(false)
...
function confirmDelete() {
  ...
  if (deleteWithTasks.value) {
    // 删除关联任务
    taskStore.tasks
      .filter((t) => t.goalId === deleteTarget.value!.id)
      .forEach((t) => taskStore.removeTask(t.id))
  } else {
    // 解除关联
    taskStore.tasks
      .filter((t) => t.goalId === deleteTarget.value!.id)
      .forEach((t) => taskStore.updateTask(t.id, { goalId: null }))
  }
  goalStore.deleteGoal(deleteTarget.value.id)
      ...
      <div v-if="deleteModal" class="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" @click.self="deleteModal=false">
        ...
        <div class="relative bg-white border border-border rounded-2xl w-full max-w-sm mx-4 shadow-2xl overflow-hidden">
          ...
          <div class="p-5 space-y-3">
            ...
            <div v-if="linkedTaskCount > 0" class="space-y-2">
              <p class="text-xs text-text-muted">该目标关联了 {{ linkedTaskCount }} 个任务</p>
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" v-model="deleteWithTasks" class="rounded" />
                <span class="text-sm text-text-primary">同时删除关联任务</span>
              </label>
              <p v-if="!deleteWithTasks" class="text-xs text-text-muted">不勾选则解除关联（任务保留，goalId 清空）</p>
            </div>
          </div>
          <div class="flex justify-end gap-2 px-5 py-4 border-t border-border bg-gray-50/50">

...
async function handleFileUpload(event: Event) {
  ...
  for (const file of Array.from(files)) {
    ...
    if (result.content && !result.content.startsWith('data:')) {
      await generatePlanPreview(result.content, file.name)
    }
    if (result.warning) {
...
async function generatePlanPreview(content: string, sourceFile: string) {
  ...
  if (hasAIConfig()) {
    ...
    try {
      const draft = await aiParseToPlanDraft(content, sourceFile, planId)
...
<template>
  <div class="space-y-6 pb-20 md:pb-0 max-w-2xl">
    ...
    <!-- AI 分析状态 -->
    <div v-if="aiAnalyzing" class="bg-gradient-to-r from-accent/5 to-blue-50/30 border border-accent/15 rounded-2xl p-5">
      <div class="flex items-center gap-3">
        <span class="text-lg animate-pulse">🤖</span>
        <div>
          <p class="text-sm font-semibold text-text-primary">AI 正在分析文档...</p>
          <p class="text-xs text-text-muted">提取目标、任务和日程安排</p>
        </div>
      </div>
    </div>

    <!-- AI 错误提示 -->
    <div v-if="aiError && !aiAnalyzing" class="bg-amber-50 border border-amber-200 rounded-xl p-4">
      <p class="text-xs text-amber-700">⚠ AI 解析失败：{{ aiError }}</p>
      <p class="text-xs text-amber-600 mt-1">已回退到本地规则解析</p>
    </div>

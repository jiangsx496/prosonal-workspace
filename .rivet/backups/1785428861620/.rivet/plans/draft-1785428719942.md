...
- `src/stores/goals.ts`：
  - load() 迁移：保留旧字段，plan 字段透传
  - 新增 computed/methods：`goalPlan(goalId)`、`goalTodayTasks(goalId)`（从 Plan.days[today] 取 taskIds → 查 TaskStore）、`goalPlanProgress(goalId)`（基于 Plan 任务的完成率）

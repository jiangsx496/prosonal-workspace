import type { PlanDraft, DraftDay, DraftBlock, DraftTask } from '@/types/planDraft'
...
export function parseToPlanDraft(content: string, _sourceFile: string, planId: string): PlanDraft {
  ...
  function ensureBlock(dayData: { blocks: Map<string, DraftBlock> }, category: string, time?: string): DraftBlock {
    const key = category + (time || '')
    if (!dayData.blocks.has(key)) {
      dayData.blocks.set(key, { id: generateDraftId('blk'), category, time, tasks: [] })
    }
    return dayData.blocks.get(key)!
  }

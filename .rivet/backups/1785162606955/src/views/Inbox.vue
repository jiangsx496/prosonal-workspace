...
function doConvert() {
  ...
  if (convertType.value === 'task') {
    ...
    goalStore.addGoal({id:goalStore.generateId(),title:content,description:'',category:convertForm.value.category,startDate:new Date().toISOString().slice(0,10),deadline:new Date(Date.now()+30*86400000).toISOString().slice(0,10),progress:0,status:'active',autoSchedule:false} as Goal)
  } else {

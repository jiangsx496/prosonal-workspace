...
const focusRemaining = ref(FOCUS_DEFAULT)
const focusRunning = ref(false)
const focusTaskId = ref<string | null>(null)
let focusTimer: ReturnType<typeof setInterval> | null = null

const focusDisplay = computed(() => {
  const m = Math.floor(focusRemaining.value / 60)
  const s = focusRemaining.value % 60
  return `${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`
})

function startFocus(taskId?: string) {
  focusTaskId.value = taskId || null
  focusRemaining.value = FOCUS_DEFAULT
...
function completeFocus() {
  stopTimer()
  const elapsed = FOCUS_DEFAULT - focusRemaining.value
  if (elapsed > 0) {
    const nowIso = new Date().toISOString()
    focusStore.addSession({
      id: focusStore.generateId(),
      taskId: focusTaskId.value,
      goalId: focusTaskId.value ? (taskStore.tasks.find((t) => t.id === focusTaskId.value)?.goalId || null) : null,
      duration: elapsed,
...
<template>
  <div class="space-y-5 pb-20 md:pb-0">
    ...
    <!-- ========== 2. 当前行动 + 专注计时 ========== -->
    <div class="bg-gradient-to-r from-accent/10 to-blue-50 border border-accent/20 rounded-2xl p-5">
      <div class="flex items-center gap-4 mb-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p v-if="nextAction.goalId && goalName(nextAction.goalId)" class="text-xs text-accent/70 mt-0.5">🎯 {{ goalName(nextAction.goalId) }}</p>
        </div>
        <button
          v-if="!focusRunning && focusRemaining === FOCUS_DEFAULT"
          class="px-4 py-2 rounded-lg bg-accent text-white text-sm font-medium hover:bg-accent-hover transition-colors shrink-0"
          @click="startFocus()"
        >🍅 开始专注</button>
      </div>
      <div v-if="focusRunning || focusRemaining < FOCUS_DEFAULT" class="flex items-center gap-4 bg-white/60 rounded-xl px-4 py-3">
        <span class="text-3xl font-bold tabular-nums text-text-primary">{{ focusDisplay }}</span>
        <div class="flex gap-2 flex-1">
          <button v-if="focusRunning" class="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 text-xs font-medium hover:bg-amber-100 transition-colors" @click="pauseFocus">⏸ 暂停</button>
          <button v-else class="px-3 py-1.5 rounded-lg bg-green-50 text-green-700 text-xs font-medium hover:bg-green-100 transition-colors" @click="resumeFocus">▶ 继续</button>
          <button class="px-3 py-1.5 rounded-lg bg-accent text-white text-xs font-medium hover:bg-accent-hover transition-colors" @click="completeFocus">✓ 完成</button>
        </div>
      </div>
      <div class="flex items-center gap-2 mt-2 text-xs text-text-muted">
        <span>🍅 今日专注：{{ focusStore.formatFocusTime() }}</span>
        <span v-if="focusStore.todaySessions.length > 0">· {{ focusStore.todaySessions.length }} 次</span>
      </div>
    </div>
    ...
    <div class="bg-card border border-border rounded-xl p-5">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-text-primary">📋 今日任务</h3>

...
function submit() {
  ...
  if (props.task) {
    ...
    // 编辑后：scheduledDate=今天 → 自动入 DailyPlan
    if (form.value.scheduledDate === today) dailyStore.addTaskToToday(props.task.id)
  } else {
    const id = store.generateId()
    store.addTask({ id, ...base, createdAt: today } as Task)
    // 新建：scheduledDate=今天 → 自动入 DailyPlan
    if (form.value.scheduledDate === today) dailyStore.addTaskToToday(id)
  }
  emit('close')
}

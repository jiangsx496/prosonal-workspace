...
export async function aiParseToPlanDraft(
    ...
    try {
      ...
      if (lastBrace > 0) {
        try {
          ...
          throw new Error(`AI 返回的 JSON 格式有误，无法解析：${e3.message}。请更换模型或简化需求后重试`)
        }
      } else {
        throw new Error(`AI 返回的 JSON 格式有误，无法解析：${e1.message}。请更换模型或简化需求后重试`)
      }
    }
  }
  ...
  const days: DraftDay[] = (parsed.days || []).map((d: any) => {
    const dayNum = (typeof d.day === 'number' && d.day >= 1) ? d.day : 1
  ...
  if (days.length < 2) {
    throw new Error('AI 生成的计划天数不足。请更详细描述需求，如"我要准备前端实习面试，重点复习 JS 和 Vue，2 周时间"')
  }
  if (allTaskCount < 6) {
    throw new Error(`AI 只生成了 ${allTaskCount} 个任务，远不够一个完整计划。请更详细描述需求后重试`)
  }
  // 检查是否有口号式任务（用户输入直接作为标题）
  const userGoalShort = content.trim().slice(0, 20)
  const hasSlogan = days.some((d) => d.blocks.some((b) => b.tasks.some((t) =>
    t.title === userGoalShort || t.title === content.trim()
  )))
  if (hasSlogan) {
    throw new Error('AI 把你的需求原样复制成了任务，没有拆解。请更详细描述需求后重试')
  }

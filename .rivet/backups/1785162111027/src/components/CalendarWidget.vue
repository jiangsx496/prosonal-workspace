...
<template>
  <div :class="embedded ? '' : 'bg-card border border-border rounded-2xl overflow-hidden'">
    <!-- 折叠头部 -->
    <button class="w-full flex items-center justify-between p-4 hover:bg-card-hover rounded-xl transition-colors" @click="expanded = !expanded">
    ...
    <!-- 展开内容 -->
    <div v-if="expanded" :class="embedded ? 'mt-2 pt-4 border-t border-border space-y-4' : 'border-t border-border p-5 space-y-4'">
      <!-- 月视图 -->
      <div>

...
- `confirmCreate()` 改造：创建 Task 后，将 PlanDraft.days/blocks 转为 Plan 结构（taskIds 引用已创建的 Task.id），存入 Goal.plan。不再丢弃层级。
- Inbox.vue 不需改（它调 confirmCreate，结果不变）

验证：`tsc --noEmit`；手动验证路径——AI 生成后 Goal.plan 有值

### Wave 3：GoalDetail 计划视图
文件：`src/views/GoalDetail.vue`

- 新增「计划视图」区域：若 goal.plan 存在，按 Day→Block 展示层级（复用 Inbox.vue 的预览样式），每个 Block 内按 taskIds 查 Task 展示标题+完成状态
...
- 今日任务数据源改造：`todayTasks` 合并两源——(a) DailyPlan（手动/兼容任务）(b) 所有 Goal.plan 中 date=today 的 taskIds
...
- 日历日期详情：选中日期的 Task 列表增加 Plan 来源——遍历所有 Goal.plan.days，找 date=selectedDate 的 taskIds，补充到选中日期的 Task 列表

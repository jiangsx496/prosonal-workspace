...
async function startAIPlanning() {
  ...
  try {
    ...
    const draft = parsePlan(userInput.value || '', 'text')
    draftStore.setDraft(draft)
  } finally {
    ...
    <div class="bg-gradient-to-br from-accent/5 to-blue-50/30 border border-accent/15 rounded-2xl p-6">
      <div class="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p class="text-xs text-text-muted">上传文件，或者告诉我你的计划，AI 帮你整理成目标、任务和日程</p>
        </div>
      </div>
      ...
      <textarea
        v-model="userInput"
        rows="3"
        placeholder="告诉我你的计划... 例如：我要准备前端实习"

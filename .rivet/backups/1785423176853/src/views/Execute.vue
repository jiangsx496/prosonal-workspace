      ...
      <div v-else-if="feedStore.hasFeed" class="space-y-4">
        ...
        <div v-if="feedStore.todayQuestions.length > 0">
          ...
          <div class="space-y-1.5">
            ...
            >
              <summary class="text-sm text-text-primary cursor-pointer select-none">{{ q.question }}</summary>
              <p class="text-xs text-text-muted mt-2 pl-3 border-l-2 border-accent/30 whitespace-pre-wrap">{{ q.answer }}</p>
            </details>
          </div>
        </div>
      </div>

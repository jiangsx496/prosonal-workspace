...
<template>
  <div class="space-y-5 pb-20 md:pb-0">
    ...
    <div class="bg-card border border-border rounded-xl p-5">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-text-primary">📰 Daily Feed</h3>
        <button
      ...
      <div v-else-if="feedStore.hasFeed" class="space-y-4">
        ...
        <div v-if="feedStore.todayRepos.length > 0">
          <p class="text-xs font-medium text-text-secondary mb-2">🔥 GitHub Trending</p>
        ...
        <div v-if="feedStore.todayQuestions.length > 0">
          <p class="text-xs font-medium text-text-secondary mb-2">📚 Daily Interview Questions</p>
          <div class="space-y-1.5">
            <details
              v-for="(q, idx) in feedStore.todayQuestions"
              :key="idx"
              class="py-1.5 px-2 rounded-lg hover:bg-card-hover transition-colors group"
            >
              <summary class="text-sm text-text-primary cursor-pointer select-none flex items-center justify-between">
                <span>{{ q.question }}</span>
                <button
                  class="text-xs text-accent opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2"
                  @click.prevent="addQuestionToTasks(q)"
                >+ Add to plan</button>
              </summary>
      ...
      <div v-else class="py-4 text-center text-xs text-text-muted">
        <span>No content yet. Click refresh to load.</span>
      </div>
    </div>

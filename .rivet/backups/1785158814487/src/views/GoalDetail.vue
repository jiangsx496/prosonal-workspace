...
// ---- 安排到今天：将未完成关联任务批量加入 DailyPlan ----
const scheduling = ref(false)
const scheduleMsg = ref('')

function scheduleToToday() {
  const tasks = undoneTasks.value
  if (tasks.length === 0) {
    scheduleMsg.value = '没有待安排的任务'
    return
  }
    ...
    <div class="bg-card border border-border rounded-2xl p-6">
      ...
      <div class="mt-4 flex items-center gap-3">
        <button
          ...
          :disabled="scheduling || undoneTasks.length === 0"
          @click="scheduleToToday"
        >📅 安排到今天（{{ undoneTasks.length }} 个任务）</button>

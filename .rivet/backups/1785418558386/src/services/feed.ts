...
export async function fetchDailyFeed(today: string): Promise<DailyFeed> {
  ...
  // 并行获取 GitHub + AI（生成 3 道题）
  const [repos, q1, q2, q3] = await Promise.allSettled([
    fetchTrendingRepos(),
    fetchDailyQuestion(),
    fetchDailyQuestion(),
    fetchDailyQuestion(),
  ...
  const questions: FeedQuestion[] = [q1, q2, q3]

...
- [ ] 新建 `src/types/plan.ts`：定义 PlanDay、PlanBlock、Plan 接口 + 工具函数（getDayByDate、getTodayDay、flattenPlanTaskIds）
- [ ] `src/mock/goals.ts`：Goal 接口加 `plan?: Plan`，mockGoals 不变（旧数据无 plan）
- [ ] `src/stores/goals.ts`：load() 迁移保留旧字段 + plan 透传
- [ ] `src/stores/goals.ts`：新增 `goalPlan(goalId)` / `goalTodayTasks(goalId)` / `goalPlanProgress(goalId)` 方法
- [ ] `src/stores/goals.ts`：deleteGoal cascade 时清理 Plan 中的 taskId 引用
...
- [ ] `src/stores/planDraft.ts` confirmCreate()：创建 Task 后将 PlanDraft.days/blocks 转为 Plan 结构（taskIds 引用已创建的 Task.id），存入 Goal.plan
- [ ] 不再丢弃层级——clearDraft() 仍清理草稿，但 Plan 已存入 Goal

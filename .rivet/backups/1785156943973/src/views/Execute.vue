    ...
    <div class="grid gap-5 lg:grid-cols-2">
      ...
      <div class="space-y-5">
        ...
        <div class="bg-card border border-border rounded-xl p-5">
          ...
          <!-- 主推进目标：高亮卡片 -->
          <router-link v-if="goalStore.activeGoals.length > 0" :to="`/goals/${goalStore.activeGoals[0].id}`" class="block bg-gradient-to-br from-accent/5 to-blue-50/50 border border-accent/15 rounded-xl p-4 mb-4 hover:border-accent/30 transition-colors">
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center gap-2 flex-1 min-w-0">
                <span class="text-xl">{{ goalStore.goalIcon(goalStore.activeGoals[0].category) }}</span>
                <div class="min-w-0">
                  <p class="text-sm font-semibold text-text-primary truncate">{{ goalStore.activeGoals[0].title }}</p>
                  <p class="text-xs text-text-muted">{{ goalStore.daysLeft(goalStore.activeGoals[0].deadline).text }}</p>
                </div>
              </div>
              <div class="text-right shrink-0">
                <p class="text-2xl font-bold" :class="goalStore.goalProgress(goalStore.activeGoals[0].id)>=100?'text-green-500':'text-accent'">{{ goalStore.goalProgress(goalStore.activeGoals[0].id) }}%</p>
                <p class="text-xs text-text-muted">{{ goalStore.goalDoneCount(goalStore.activeGoals[0].id) }}/{{ goalStore.goalTaskCount(goalStore.activeGoals[0].id) }}</p>
              </div>
            </div>
            <div class="h-2 bg-white/60 rounded-full overflow-hidden mb-3">
              <div class="h-full rounded-full transition-all duration-500" :style="{width:goalStore.goalProgress(goalStore.activeGoals[0].id)+'%',backgroundColor:goalStore.progressColor(goalStore.goalProgress(goalStore.activeGoals[0].id))}"></div>
            </div>
            <div class="flex items-center gap-2 bg-white/50 rounded-lg px-3 py-2">
              <span class="text-sm shrink-0">→</span>
              <p class="text-xs font-medium text-text-primary truncate flex-1">{{ goalStore.goalNextTask(goalStore.activeGoals[0].id) }}</p>
            </div>
          </router-link>
          <!-- 其余目标：简洁列表 -->
          <div v-if="goalStore.activeGoals.length > 1" class="space-y-3">
            <router-link v-for="g in goalStore.activeGoals.slice(1, 3)" :key="g.id" :to="`/goals/${g.id}`" class="block group">
              <div class="flex items-center gap-2 mb-1">
                <span class="text-base">{{ goalStore.goalIcon(g.category) }}</span>
                <span class="text-sm font-medium text-text-primary truncate flex-1 group-hover:text-accent transition-colors">{{ g.title }}</span>
          ...
          <div v-if="goalStore.activeGoals.length === 0" class="flex flex-col items-center justify-center py-6 text-text-muted text-sm">
            <span>还没有进行中的目标</span>

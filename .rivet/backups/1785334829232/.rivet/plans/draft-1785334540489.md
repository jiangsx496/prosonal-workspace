...
| 来源 | 形态 | 消费者 |
|------|------|--------|
| `mock/planDraft.ts` | `{ id, source, title, goal, tasks:[{id,title,priority,selected,dueDate}], schedule, summary }` | aiParser.ts、stores/planDraft.ts、Inbox.vue |
| `stores/plans.ts` | `{ planId, goalTitle, goalDescription, startDate, endDate, totalDays, tasks:[{title,day,date,time,priority}], rawContent }` | ai.ts、planParser.ts |

Inbox.vue 调用 `draftStore.currentDraft`（store 未导出该 ref，内部名为 `draft`）和 `draftStore.createFromAI()`（store 中不存在该方法）——当前代码处于编译不通过的断裂状态。

## 统一层级模型设计

新建 `src/types/planDraft.ts` 作为唯一类型源，取代上述两套定义：
...
### 1. `services/ai.ts`（AI API 路径）— 改 SYSTEM_PROMPT + 输出结构
...
### 2. `services/aiParser.ts`（本地启发式路径）— 改输出结构
...
### 3. `services/planParser.ts`（v2 day-based 路径）— 已有 day 结构，适配输出

已有 `currentDay` / `dayDates` 逻辑，改动最小：把扁平 `tasks[]` 重组为 `days[].blocks[].tasks[]`，一个 Day 一个 block（按 category 粗分或统一归入 "学习" block）。

### 类型导入统一

所有解析器从 `@/types/planDraft` 导入，删除 `mock/planDraft.ts` 和 `stores/plans.ts` 中的 PlanDraft 相关类型定义。

## Store 层适配

### `stores/planDraft.ts` 重构
...
- [ ] T2. `stores/daily.ts` 增加 `addTaskToDate` 方法
- [ ] T3. 删除 `mock/planDraft.ts` 中的类型定义（保留文件做 re-export 或直接删）

...
export const useGoalStore = defineStore('goals', () => {
  ...
  function deleteGoal(id: string, strategy: 'cascade' | 'detach' = 'cascade') {
    ...
    const linkedTasks = taskStore.tasks.filter((t) => t.goalId === id)

    if (strategy === 'cascade') {
      // 级联删除关联任务（removeTask 内建 daily plan 清理）
      linkedTasks.forEach((t) => taskStore.removeTask(t.id))
    } else {
      // 仅解除关联
      linkedTasks.forEach((t) => taskStore.updateTask(t.id, { goalId: null }))
    }

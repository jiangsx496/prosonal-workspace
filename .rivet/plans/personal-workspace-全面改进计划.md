...
> **Status: APPROVED** — 2026-07-30T13:02:09.816Z
...
| `src/services/planParser.ts` | `parseToPlanDraft` 无消费者 | Inbox.vue 走的是 aiParser.ts 的 `parsePlan`，planParser.ts 全文件可标注废弃 |
...
| 文件 | 操作 | 修复 |
|------|------|------|
| `src/services/parser.ts:45-55` | 修改 | `let scheduledDate` 声明为 `string`；`let priority` 声明为 `Task['priority']`。注意：parser.ts 已无消费者（grep 确认），可考虑直接删除整文件 |
| `src/views/Tasks.vue:55-63` | 修改 | 删除重复的 `'deferred'` key，删除不存在的 `'today'` key |
| `src/views/Habits.vue:12` | 修改 | `frequency` 去掉 `as const`，或改为 `'daily' as Habit['frequency']` |
| `src/services/scheduler.ts:33` | 修改 | 删除未使用的 `today` 变量 |
| `src/stores/reminders.ts:40` | 修改 | 删除未使用的 `base` 变量 |
| `src/services/parser.ts` | 删除 | 整文件删除——grep 确认无消费者（无 import 引用），是旧版解析器被 aiParser.ts 取代 |

**验证**：`npx vue-tsc --noEmit -p tsconfig.app.json` 输出 0 行

### Wave 2: 死代码清理（不阻塞 build，改善可维护性）

| 文件 | 操作 | 说明 |
|------|------|------|
| `src/mock/planDraft.ts` | 删除 | grep 确认 0 消费者（`from '@/mock/planDraft'` 返回 0 匹配） |
| `src/stores/plans.ts:15-31` | 修改 | 删除 `PlanDraft`/`PlanDraftTask` 接口（已被 `src/types/planDraft.ts` 取代），保留 `Plan` 接口和 store |
| `src/services/planParser.ts` | 标注废弃 | 无消费者，但 aiParser.ts 内有同名函数 `summarizeDraft`/`checkDraftQuality`——确认不冲突后可删除 |
...
**已验证假设**：
- `parseToPlanDraft`（`src/services/planParser.ts:47`）无直接消费者——grep 确认只有定义和自身导出的 `summarizeDraft`/`checkDraftQuality`，没有任何 import 引用。`planParser.ts` 全文件可标注为废弃。
- Habit 接口的 `frequency` 字段已是 `'daily' | 'weekly'` 联合类型（`src/mock/habits.ts:5`）。错误出在 `Habits.vue:12` 用 `'daily' as const` 把类型收窄。

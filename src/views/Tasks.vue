  ...
  <div class="space-y-6 pb-20 md:pb-0">
    ...
    <div v-if="store.filteredTasks.length > 0" class="flex items-center gap-3 flex-wrap">
      ...
      <template v-if="selectedIds.size > 0">
        <span class="text-xs text-text-muted">已选 {{ selectedIds.size }} 项</span>
        <button class="px-2.5 py-1 rounded-lg bg-green-50 text-green-700 text-xs font-medium hover:bg-green-100 transition-colors" @click="batchComplete">✓ 批量完成</button>
        <button class="px-2.5 py-1 rounded-lg bg-accent/10 text-accent text-xs font-medium hover:bg-accent/20 transition-colors" @click="batchScheduleToday">📅 安排今天</button>
        <button class="px-2.5 py-1 rounded-lg bg-red-50 text-red-600 text-xs font-medium hover:bg-red-100 transition-colors" @click="batchDelete">🗑 批量删除</button>
      </template>
    </div>
